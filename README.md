...
Основной способ запуска — **Docker Compose**. Нужен только Docker с плагином
`compose` (или podman с `podman compose`); Python, Node и PostgreSQL на
машине не нужны.
...
1. собирается образ приложения: фронт (`npm run build`) и Python-окружение;
2. `db` — PostgreSQL 17 — инициализируется схемой и данными датасета
...
| Команда | Что делает |
|---|---|
| `docker compose ps` | состояние сервисов (`db`, `app`, `prometheus`, `grafana` должны быть `healthy`) |
...
`.env` другие: `HTTP_PORT=8080`, `HTTPS_PORT=8443`, и открыть
<https://localhost:8443>.

**Демонстрация полного цикла квартала.** После запуска база содержит только
базовый план. Чтобы показать пересчёт, на экране «Загрузка» по очереди
загрузить факт спринтов 1, 2 и 3 из папки `demo/`
(`actuals_sprint_1.csv` … `actuals_sprint_3.csv`) — после каждой загрузки план
пересчитывается, появляются алерты и KPI по факту.

> Файлы в `demo/` — **синтетический** факт для демонстрации пересчёта, а не
> данные организаторов. Реальный факт загружается по шаблону (кнопка «Шаблон»).

Подробнее о стеке, мониторинге и бэкапах — [раздел 12](#12-docker-мониторинг-бэкапы).

### 1.2. Без Docker для приложения (`run.sh`)

Запасной вариант для машины без Docker Compose или для отладки бэкенда:
...
перезалить базу с нуля, `--no-build` — не собирать фронт; порт —
`APP_PORT=8080 ./run.sh`. Остановить — `Ctrl+C`.
...
```
docker-compose.yaml     запуск: полный стек; Dockerfile — образ приложения; Caddyfile — прокси
...
| KPI без факта, риски почти пустые | факт ещё не загружен — это нормально. Для демонстрации — загрузить файлы из `demo/` |
...
| `run.sh`: `Address already in use` на 8000 | сервер уже запущен: остановить его или `APP_PORT=8080 ./run.sh` |
